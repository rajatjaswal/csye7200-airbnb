...
class ListingSpec extends FlatSpec with Matchers{
  ...
  "Listing" should """return true if is withinMileOfArea of a PopularArea of same coordinates """  in
    {
      ...
      val tuple = Helper.isWithinMileOfArea(HelperSpec.pop1, lat1, lat2)
  ...
  it should """return closest PopularArea if there are multiple within 1 mile but average price lies oustide range """  in
  {
    val lat1 = -37.866
    val lat2 = 144.990
    val listing = HelperSpec.listing.copy(latitude = lat1, longitude = lat2)
    val tuple = listing.hasClosestPopularArea(listing, HelperSpec.getPopularAreas())
    tuple should matchPattern {
      case (_,0, _) =>
    }
  ...
  it should """return null as closestPopularArea if there is none closest PopularArea within 1 mile """  in
  {
    val lat1 = -38.900
    val lat2 = 145.990
    val listing = HelperSpec.listing.copy(latitude = lat1, longitude = lat2, avgPrice = 60)
    val tuple = listing.hasClosestPopularArea(listing, Seq.empty)
    tuple should matchPattern {
      case (null,0, _) =>
    }
  }
