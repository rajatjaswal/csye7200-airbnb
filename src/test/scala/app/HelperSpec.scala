...
object HelperSpec {
  ...
  val address1 = HouseAddress.parse("Abbotsford,121/56 Nicholson St,2,u,,PI,Biggin,7/11/2016,2.5,3067,2,2,1,4292,82,2009,Yarra City Council,-37.8078,144.9965,Northern Metropolitan,4019".split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)", -1), getPopularAreas())
  val address2 = HouseAddress.parse("Altona,2/6 Galvin St,3,t,650000,SP,Williams,12/11/2016,13.8,3018,,,,,,,Hobsons Bay City Council,,,Western Metropolitan,5301".split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)", -1), getPopularAreas())
...
class HelperSpec extends FlatSpec with Matchers {
  ...
  it should "return different house nightly prices for different total price" in {
    ...
    val expectedNightlyPrice = Seq(44,73,136,176,194,296,493)
    prices.foreach( p => {
  ...
  it should """return new listings with popularArea field Injected in them"""  in
    {
      ...
      assert(listings.tail.head.get.isWithinPopular == 1)
    }
}